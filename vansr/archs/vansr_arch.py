# -----------------------------------------------------------------------------------
# VANSR: Super Resolution Using VAN, https://arxiv.org/pdf/2202.09741.pdf
# Originally Written by Meng-Hao Guo, Modified by Bowen Hu.
# -----------------------------------------------------------------------------------
...
class OverlapPatchEmbed(nn.Module):
    ...
    def forward(self, x):
        x = self.proj(x)
        _, _, H, W = x.shape
        #x = self.norm(x)  #sr could not need norm at there      
        return x, H, W
...
class VANSR(nn.Module):
    def __init__(self, img_size=224, in_chans=3, scale = 2, embed_dims=[64, 128, 256, 512],
                mlp_ratios=[4, 4, 4, 4], drop_rate=0., drop_path_rate=0., #norm_layer=nn.LayerNorm,
                 depths=[3, 4, 6, 3], num_stages=4, flag=False,img_range=1):
        super().__init__()
        num_feat = 64
        if in_chans == 3:
        ...
        for i in range(num_stages):
            patch_embed = OverlapPatchEmbed(img_size=img_size,# if i == 0 else img_size // (2 ** (i + 1)),
                                            ...
                                            stride=1,#4 if i == 0 else 2,
                                            in_chans=num_feat if i == 0 else embed_dims[i - 1],
        ...
        self.conv_before_upsample = nn.Sequential(nn.Conv2d(embed_dims[-1], num_feat, 3, 1, 1),
                                                      nn.LeakyReLU(inplace=True))
        # sr tail
        self.tail = Upsample(scale, num_feat)
...
class DWConv(nn.Module):
    def __init__(self, dim=768):
        ...
        #self.dwconv = DepthWiseConv2dImplicitGEMM(dim, 7, bias=True)
